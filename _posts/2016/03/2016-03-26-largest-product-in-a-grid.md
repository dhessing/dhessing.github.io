...
It doesn't check for all directions, because we'd check certain sequences twice.
To see this, consider starting from `[0 0]` and going right. We get `(08 02 22 97)`.
This is the same combination of numbers as going from `[0 3]` to the left but in reverse: `(97 22 03 08)`.
The product of these would be exactly the same.

Next up we convert these coordinates into values of the grid. The grid here is represented as a nested vector.
...
`get-in` will grab the values for us, or will supply 0 if we go over the edge.
See for example `(8 0 0 0)` in the following call:

    (lines 0 0)
    => ((8 2 22 97) (8 49 31 23) (8 49 81 52) (8 0 0 0))
    
This is fine because these will become zero when multiplied and will basically never be considered as the highest number.
    
To get the answer we just loop over all the points of the grid and call `lines` on it.
We multiply these, call `max` and we're done!
